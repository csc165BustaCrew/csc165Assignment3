...
public class Camera3PMouseKeyboard {
	private ICamera cam;
	...
	public Camera3PMouseKeyboard(ICamera cam, SceneNode target, IInputManager inputMgr, String controllerName){
		this.cam = cam;
		this.target = target;
	...
	private void setupInput(IInputManager inputMgr, String controllerName) {
		...
		inputMgr.associateAction(controllerName, Axis.X, orbitAction, REPEAT_WHILE_DOWN);
		inputMgr.associateAction(controllerName, Axis.Z, radAction, REPEAT_WHILE_DOWN);
		inputMgr.associateAction(controllerName, Axis.Y, elevAction, REPEAT_WHILE_DOWN);
		inputMgr.associateAction(inputMgr.getKeyboardName(), Identifier.Key.SPACE, resetAction, ON_PRESS_ONLY);
